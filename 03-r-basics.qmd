---
output-file: r-primer-syntax-and-basic-functions
---

# R Primer (Syntax and Basic Functions)

## Overview of Approach
This tutorial is written as a guide for the question, “How do I do X in R?” It is a reference chapter rather than a script to run from top to bottom. Work through one section at a time, inspect what each line creates, and adapt the examples to a small project of your own.

By the end of the chapter, you should be able to recognize common R object types, move data into and out of R, combine datasets, manipulate strings, calculate descriptive statistics, and verify that a file has not changed.

## Installing packages

Packages extend base R with specialized functions. Installation usually happens once per computer; loading a package with `library()` happens once per R session. Keeping those two operations separate makes scripts faster and easier to reproduce.
```
...
Assignment gives an object a name so it can be reused. Descriptive names cost a few extra keystrokes but make later transformations much easier to understand.
...
An object's class determines which operations are valid. When a function behaves unexpectedly, checking `class()` is often the fastest first diagnostic.
```r
...
Reproducible scripts should discover and create paths deliberately instead of depending on files opened by hand. The following functions list files, check for directories, and create temporary project structure.
...
Research data arrive in many formats. Prefer formats that preserve the information you need while remaining readable by collaborators, and always confirm that imported column types match your expectations.
...
An Excel workbook can be useful for sharing related tables with collaborators who do not use R. Named sheets preserve the relationship among tables without forcing them into one oversized spreadsheet.
```r
library(rio)
...
Human-friendly survey labels often contain spaces, punctuation, and inconsistent capitalization. Cleaning names once near import prevents repeated quoting and reduces typing errors later.
...
Binning converts a continuous measure into intervals. It can simplify a display or implement a substantively meaningful threshold, but it also discards variation. Keep the original continuous variable alongside the binned version.
...
Text cleaning commonly requires two distinct operations: finding patterns and replacing them. Test patterns on a few known examples before applying them to an entire column.
```r
...
Simulated data are useful for testing a workflow before sensitive or slow-to-access data are available. Setting a random seed earlier in the script makes the same simulated values reproducible.
...
Descriptive statistics provide a compact first check of center, spread, range, and unusual values. They should accompany—not replace—inspection and visualization of the distribution.
...
A structured quality report helps identify missingness, implausible ranges, and unexpected types before modeling. Save the report when it forms part of the documented decision trail.
...
The hash is a fingerprint, not an encryption method. Matching hashes provide strong evidence that two copies contain the same bytes; a mismatch tells you the files differ but not why.
...
The individual functions in this chapter are building blocks. A durable workflow connects them in a visible sequence: inspect inputs, transform deliberately, check intermediate results, save outputs, and record enough metadata for someone else—or your future self—to repeat the work.
